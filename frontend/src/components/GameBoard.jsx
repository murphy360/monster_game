...
export default function GameBoard({
  ...
  const namedSprites = useMemo(
    () =>
      spriteUrls.reduce((acc, spriteUrl, idx) => {
  ...
  useEffect(() => {
    ...
    const getRandomDelay = () => {
      const avg =
        performanceRef.current.resolvedCount > 0
      ...
      return Math.max(SPAWN_DELAY_MIN_MS, Math.min(SPAWN_DELAY_MAX_MS, randomizedDelay));
    };
    ...
    const spawnOne = () => {
      setActiveSpawns((prev) => {
        ...
        const availableSprites = uniqueSpriteUrls.filter(
          (spriteUrl) => !visibleSprites.has(spriteUrl),
        );
        ...
        const chosenWindowEntry =
          spawnableWindows[Math.floor(Math.random() * spawnableWindows.length)];
    ...
    <div className="game-board-wrapper">
      {showScore && (
        <div className="score-bar">
          ...
          <div className={`timer-display${roundOver ? ' done' : ''}`}>Time: {secondsLeft}s</div>
        </div>
      )}
      <div
        ref={boardRef}
      ...
      >
        ...
        {overlayUrl && (
          <img src={overlayUrl} alt="Building overlay" className="game-board-overlay-image" />
        )}
